...
# 加载数据
df = pd.read_csv(root_path+'photo_info.tsv',sep='\t')
df['keyword_segment'].apply(lambda x:"".join(eval(x)))
search_session_id_list = df['photo_id'].tolist()
keyword_segment_list = df['caption'].tolist()
...
# 保存句向量
with open(root_path+'reco2embedding.pickle', 'wb') as f:
